...
{{--<div class="form-group has-float-label col-sm-12">--}}
    {{--<label for="jobcat_id">Job Category <span class="asterisk">*</span></label>--}}
    {{--{{ Form::select('jobcat_id', $jobCategory, old('jobcat_id'), ['class' => 'form-control ' . ($errors->has('jobcat_id') ? 'redborder' : '') ]) }}--}}
    {{--<small class="text-danger">{{ $errors->has('jobcat_id') ? $errors->first('jobcat_id') : '' }}</small>--}}
{{--</div>--}}

<input type="hidden" value="3" name="entity_id">
<input type="hidden" value="1" name="jobcat_id">
...
<div class="form-group has-float-label col-sm-12">
    <label for="title_ar">Job Location </label>
    {{ Form::text('location', old('location'), ['placeholder' => 'Job Location', 'class' => 'form-control ' . ($errors->has('location') ? 'redborder' : '') ]) }}
    <small class="text-danger">{{ $errors->has('title_ar') ? $errors->first('location') : '' }}</small>
</div>
